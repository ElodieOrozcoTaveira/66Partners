...
import {
  ...
  ParticipationError,
} from "../services/participation.services.js";
...
export class ParticipationController {
  ...
  static async accept(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const participation = await ParticipationService.acceptParticipation(
        id,
        req.userId
      );
   ...
   */
  static async refuse(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.userId) {
        res.status(401).json({
          success: false,
      ...
      const participation = await ParticipationService.refuseParticipation(
        id,
        req.userId
      );
